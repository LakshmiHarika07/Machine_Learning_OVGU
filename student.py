...
def Linear_Regression_Gradient_Descent():
    ...
    file , learning_rate , threshold_rate = args.data, float(args.eta), float(args.threshold)
    #Loading dataset
    with open(file ,'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        data=[row for row in reader]
        #Initialising null list for x values
    x_values = [ ]
...
#seperating dependent and independent variables
    for i in range(0 , rows) :
        ...
        #initialising gradient list
    grad_list={ }
    for i in range(columns):
        grad_list[i]=float(0)
        #initialising weight list
    weight_list={}
    for i in range(columns):
    ...
    stopping_criteria=[]
    '''
    Starting the evaluation of Gradient_Descent
    '''
    while True:
...
#appending value of each row to the hypothesis list
            hypothesis.append(row)
#calculating error ,i.e. difference of the original prediction and hypothesis
...
#adjusting our weights
        ...
        iteration+=1
'''
Parsing the dataset using an argument parser
'''
if __name__ == '__Linear_Regression_Gradient_Descent__':
